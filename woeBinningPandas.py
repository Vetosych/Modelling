# Документация https://pypi.org/project/woeBinningPandas/#modal-close
...
def woe_binning_2 (df, target_var, pred_var, min_perc_total, min_perc_class, stop_limit, abbrev_fact_levels, bad, good):
    ...
    if len(dfrm.iloc[:,0].drop_duplicates()) == 2 and (dfrm.iloc[:,1].dtypes.kind in 'bifc') == True:
        ...
        dfrm["predictor_var_binned"] = pd.cut(dfrm["predictor_var"], np.unique(np.around(cutpoints,4)), right=True, labels = None,
            retbins=False, precision=10, include_lowest=False) # БВВ
        ...
        freq_table =freq_table.append(missing,ignore_index=True) # БВВ , sort=False
    ...
    # (in reverse order to remain correct reference to cutpoints)
        ...
        if woe_dfrm_final is None: # БВВ        
            print('У переменной сформирован только 1 бин, выбрасываем.')
            return []
        ...
        look_up_table = look_up_table[~look_up_table.index.duplicated()] # БВВ
        ...
        binning = pd.concat([(woe_dfrm_final["woe"]).reset_index(drop=False), look_up_table["cutpoints_final"].reset_index(drop=True), look_up_table["upper_cutpoints_final_dfrm"].reset_index(drop=True), look_up_table["iv_total_final"].reset_index(drop=True), look_up_table["good"].reset_index(drop=True), look_up_table["bad"].reset_index(drop=True), look_up_table["col_perc_a"].reset_index(drop=True), look_up_table["col_perc_b"].reset_index(drop=True), woe_dfrm_final["iv_bins"].reset_index(drop=True)], axis=1).set_index(["predictor_var_binned"]) # БВВ , sort=False
    ...
    if len(dfrm.iloc[:,0].drop_duplicates()) == 2 and (dfrm.iloc[:,1].dtype == 'object')==True:
        ...
        #df[pred_var+"_binned"] = df[pred_var].astype('category') # БВВ
        df.loc[:,pred_var+"_binned"] = df[pred_var].astype('category') # БВВ
...
#### Warning message and defaults in case parameters are not specified
    ...
    final_bin_result = woe_binning_2(df, target_var, pred_var, min_perc_total, min_perc_class, stop_limit, abbrev_fact_levels, bad, good)

    return final_bin_result
